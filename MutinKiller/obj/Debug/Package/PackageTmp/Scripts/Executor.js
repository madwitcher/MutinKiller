...
function QueueGroupComplete() {
    ...
    if (groupId == null || groupId == '' || groupId == undefined || isNaN(groupId) || groupId.match(/\d/g).length != groupId.length || Number(groupId) < 1) {
        ...
        $.ajax({
            type: "GET",
            url: "MutinKiller/Index/QueueObject?type=group&id=" + groupId,
            contentType: "application/json",
            success: function (result) {
                if (result.Failed) {
...
function ShowStatsDialog(stats) {
    var dlgButtons = [
            ...
            {
                text: "Start Executor",
                click: function () { StartExecutor(); }
            },
            {
                text: "Stop Executor",
                click: function () { StopExecutor(); }
            },
            {
                text: "Close",
...
function StartExecutor() {
    CloseStatsDialog();
    (waitScreen = new AGW_WaitScreen("#body", "Performing operation...", "Please wait")).Show();
    $.ajax({
        type: "GET",
        url: "MutinKiller/Index/StartExecutor",
        contentType: "application/json",
        success: function (result) {
            waitScreen.Close();
            if (result.Failed) {
                $("#divMessage").append("<br/><span style='width:100%;display:block;word-wrap:break-word;color:red'>Operation failed. Error: " + result.ErrorMessage + "</span>");
                $("#divMessage")[0].scrollTop = $("#divMessage")[0].scrollHeight;
            }
            else {
                ShowStatsDialog(result.Stats);
        ...
        error: function (error) {
            waitScreen.Close();
            $("#divMessage").append("<br/><span style='width:100%;display:block;word-wrap:break-word;color:red'>Operation failed: " + error.responseText + "</span>");
            $("#divMessage")[0].scrollTop = $("#divMessage")[0].scrollHeight;
        }
    });
}
...
function StopExecutor() {
    ...
    $.ajax({
        type: "GET",
        url: "MutinKiller/Index/StopExecutor",
        contentType: "application/json",
        success: function (result) {
            waitScreen.Close();
            if (result.Failed) {
